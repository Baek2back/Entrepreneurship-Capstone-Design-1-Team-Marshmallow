...
public class ModelRenderer implements GLSurfaceView.Renderer {
    ...
    private static final float Z_FAR = MODEL_BOUND_SIZE * 5;
    ...
    public void setAngleY(float angleY) {this.angleY = angleY;}
    ...
    private void updateViewMatrix() {
        Matrix.setLookAtM(viewMatrix, 0, 0, 0, translateZ, 0f, 0f, 0f, 0f, 1f, 0f);
    ...
    public void onDrawFrame(GL10 unused) {
        // requestRender가 호출될 때만 불려지는 메소드이다.
        GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT | GLES20.GL_DEPTH_BUFFER_BIT);
        relativeY = angleY - previousY;
        previousY = angleY;
        floor.draw(viewMatrix, projectionMatrix, light);
        if (model != null) {
            Matrix.rotateM(model.modelMatrix, 0, -relativeY, 0.0f, 1.0f, 0.0f);
